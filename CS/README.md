...
Каждый из запросов к веб-сервису должен быть подписан. Подписывается часть строки запроса, результат кодируется в формате base64 и помещается в http заголовок сообщения "X-WSALAM-SIGN".
...
Объявляем константы, которые будем использовать в коде
...
Создаем WebClient, подписываем строку запроса, результат добавляем в http header и выполняем запрос к веб-сервису
